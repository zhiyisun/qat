...
enum icp_qat_fw_comn_request_id {
	...
	ICP_QAT_FW_COMN_REQ_CPM_FW_INLINE = 11,
...
struct icp_qat_fw_comn_req_hdr_cd_pars {
	union {
		...
		struct {
			u32 serv_specif_fields[ICP_QAT_FW_NUM_LONGWORDS_4];
		} s1;
	} u;
};
...
struct icp_qat_fw_comn_req_cd_ctrl {
	u32 content_desc_ctrl_lw[ICP_QAT_FW_NUM_LONGWORDS_5];
};
...
struct icp_qat_fw_comn_req_hdr {
	...
	u8 service_type;
	u8 hdr_flags;
	u16 serv_specif_flags;
	u8 comn_req_flags;
	u8 extended_serv_specif_flags;
};

struct icp_qat_fw_comn_req_rqpars {
	u32 serv_specif_rqpars_lw[ICP_QAT_FW_NUM_LONGWORDS_13];
};
